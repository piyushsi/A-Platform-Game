...
//   Determining the coin’s actual position
class Coin {
...
//initial we have passed with vector class
...
// Drawing each actor by creating a DOM element
function drawActors(actors) {
  var coins = ["job.png", "node.png", "react.png", "html.png"];
  var i = actors.filter((a) => a.type == "coin").length - 1;
  var play = document.createElement("img");
  play.src = "player" + coins[i];
  ...
  return elt(
    ...
    ...actors.map((actor) => {
      if (actor.type == "player") {
        let rect = elt("img", { class: `actor ${actor.type}` });
        rect.src = "player" + coins[i];
...
DOMDisplay.prototype.scrollPlayerIntoView = function (state) {
  ...
  let center = player.pos.plus(player.size.times(0.8)).times(scale);
...
//   If any actor does overlap, its collide method gets a chance to update the state. Touching a lava actor sets the game status to "lost".
...
// The wobble property is incremented to track time and then used as an argument to Math.sin to find the new position on the wave.
...
//   promise to the game’s end status.
...
//   This function accepts an array of level plans (strings) and a display constructor:
async function runGame(plans, Display) {
  for (let level = 0; level < plans.length; ) {
    let status = await runLevel(new Level(plans[level]), Display);
    if (status == "won") level++;
  }
  window.location.href = "https://altcampus.io/";
}
